...
// If ur reading this, there is 100% a way to import mtls and objs just from the obj file but im tired :p
...
Blockbench.addDragHandler(
    ...
    function(files) {
        ...
        fetch(files[0].path)
            ...
            .then(function(zip) {
                return zip.file("tex.png").async("base64")
            }).then(file => {
                tex = `data:image/png;base64,${file}`;
            });
        fetch(files[0].path)
            .then(function(response) {
                return Promise.resolve(response.blob());
            ...
            .then(JSZip.loadAsync)
            .then(function(zip) {
                return zip.file("model.obj").async("string")
            }).then(file => {
                obj = `data:obj;base64,${btoa(file.replace(/(?<!vn.*\n)usemtl.*/g, '').replace(/l.*/g, '').replace(/usemtl.*/g, `$&\ns 1`))}`;
            })
        setTimeout(
            () => {
                addModel(obj, tex)
...
function addModel(obj, tex) {
    ...
    objLoader.load(obj, (root) => {
        root.traverse(function(child) {
            if (child instanceof THREE.Mesh && tex) {
